...
async def cmd_start(event):
    await event.reply(
        ...
        "✅ Supports files up to <b>2 GB</b> — no local API server needed!\n\n"
...
async def download(token: str):
    ...
    if not entry:
        raise HTTPException(
            status_code=404,
            detail="Link not found. Please re-send the file to the bot.",
        )
...
async def main():
    ...
    log.info("Bot started with Telethon (2 GB support enabled)")
